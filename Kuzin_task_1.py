...
# Индивидуальное задание 1(Кузин М.А)
...
def create_table(csv_path):
    global new_table, start_time
    interest_list = input("Введите интересующие вас команды через запятую: ")
    ...
    for i in range(len(interest_list)):
        interest_list[i] = interest_list[i].strip()
        if interest_list[i] not in teams:
            print(f"Ошибка ввода! Комада {interest_list[i]} отсутствует в общей таблице!")
            interest_list.pop(i)
        if interest_list == []:
...
def string_modification(string):
    ...
    modification_list.append(modification_list[3])
...
def get_score_of_game(s):
    ...
    goals_team2 = 0
    counter = 0
    while counter < 3 or (goals_team1 == goals_team2 and counter >= 3): # получаем список общего счета и фиксируем в каком периоде закончилась игра
    ...
    for i in range(2): # Подсчет шайб
        if data[i] > data[hi]: # Проверка победы одной команды над другой(проверяет каждую из двух команд)
            if counter == 3: # Если игра закончилось в основное время
                ...
                finish_list[hi][7] += 1 # О hi-ой команде
        finish_list[i][9] += data[i]
        finish_list[i][10] += data[hi]
        hi -= 1
        finish_list[i][8] += finish_list[i][9] - finish_list[i][10]
    return finish_list
...
def create_finish_table(array_info, new): # на вход: [['Динамо Мн', 1, 0, 0, 0, 0, 0, 3, 2, 2, 0], ['Автомобилист', 0, 0, 0, 0, 0, 1, 0, -2, 0, 2]]
    for i in range(2): # new = new_table в самом начале)))
        for j in range(len(new_table)):
            if array_info[i][0] in new_table[j]:
                n = 0
...
def main_function():
    ...
    create_table(csv_path) # Создание таблицы(со значением 0)
